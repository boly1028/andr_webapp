...
export default function useQueryAssets(
  ...
  const { loading, data, error, fetchMore, previousData, refetch } = useAssetsQuery(
    { variables: { walletAddress, limit, offset, orderBy: filters.orderBy || IAndrOrderBy.DESC, adoType: filters.adoType || undefined, search: filters.search }, notifyOnNetworkStatusChange: true },
  );
